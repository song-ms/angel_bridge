...
import logging
import grpc
import bpk_button.helloworld_pb2 as helloworld_pb2
import bpk_button.helloworld_pb2_grpc as helloworld_pb2_grpc

class BackpackButtonSubscriber(Node):

    def __init__(self, rclpy):

        super().__init__('backpack_button_subscriber')
        print('Create backpack_button_subscriber')
        self.prev_value_ = None
        self.btn_queue_ = Queue()
        self.rclpy_ = rclpy
        self.grpc_channel = grpc.insecure_channel('localhost:50051')
        self.subscription = self.create_subscription(UInt8MultiArray, '/topic/button/backpack', self.listener_callback, 10)
...
def main(args=None):
    ...
    minimal_subscriber = BackpackButtonSubscriber(rclpy)
